...
describe('SchemaResolver', () => {
    ...
    describe('buildFieldSchemas — flowchart node', () => {
        ...
        it('type field is an enum', () => {
            ...
            expect(typeField!.fieldType).toBe('enum');
            if (typeField!.fieldType === 'enum') {
                expect(typeField!.options).toContain('process');
                expect(typeField!.options).toContain('decision');
            }
        });
    ...
    describe('buildFieldSchemas with basePath', () => {
        it('prefixes field paths with basePath', () => {
            ...
            const typeField = fields.find(f => f.path === 'nodes.myNode.type');
            expect(typeField).toBeDefined();
        });
    });
